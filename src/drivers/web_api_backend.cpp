...
PingResponse web_api_backend_ping() {
    DEBUG_BACKEND("\nweb_api_backend_ping():\n");
    WEBAPI_PING_RESPONSE.valid = false;
    request("ping", false, [](HTTPClient &http, int httpCode) -> void {
        if (httpCode == HTTP_CODE_OK) {
            ...
            #endif
            if (json.success()) {
                strcpy(WEBAPI_PING_RESPONSE.uuid, json["data"]["uuid"].as<char *>());
                strcpy(WEBAPI_PING_RESPONSE.name, json["data"]["name"].as<char *>());
                WEBAPI_PING_RESPONSE.is_active = json["data"]["is_active"].as<bool>();
                WEBAPI_PING_RESPONSE.valid = true;
            } else {
    ...
    DEBUG_BACKEND("\nWEBAPI_PING_RESPONSE[valid] = %s\n", WEBAPI_PING_RESPONSE.valid ? "true" : "false");
    DEBUG_BACKEND("WEBAPI_PING_RESPONSE[uuid] = %s\n", WEBAPI_PING_RESPONSE.uuid);
